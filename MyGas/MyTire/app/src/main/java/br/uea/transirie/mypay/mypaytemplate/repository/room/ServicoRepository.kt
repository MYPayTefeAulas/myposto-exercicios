...
import br.uea.transirie.mypay.mypaytemplate.repository.ServicoDataSource
...
class ServicoRepository(database: AppDatabase): ServicoDataSource {
    ...
    fun getAllServicos(desativado: Boolean = false): List<Servico> {
        return servicoDao.getAllServicos(desativado)
    }

    override fun getAllServicosAtivos(): List<Servico> {
        return servicoDao.getAllServicosAtivos()
    }
}