...
class CaixaFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        userPrefs = (activity as AppCompatActivity?)!!.getSharedPreferences(getString(R.string.PREF_USER_DATA), Context.MODE_PRIVATE)
    ...
    private fun calcularGanhosDespesas(){
        doAsync {
            ...
            caixaViewModel.getRecebidosDia(LocalDate.now(),usuario)
            caixaViewModel.getDespesasDia(LocalDate.now(),usuario)
            uiThread {
                ...
                preference = (activity as AppCompatActivity?)!!.getSharedPreferences(PREF_VERIF_ABRIR_CAIXA, MODE_PRIVATE)
                user = preference.getString("email", "").toString()
                val statusAbrirCaixa= Gson().fromJson(preference.getString(user, Gson().toJson(
