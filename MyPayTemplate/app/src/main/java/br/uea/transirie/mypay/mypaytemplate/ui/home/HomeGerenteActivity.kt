...
class HomeGerenteActivity : AppCompatActivity() {
    private val logtag = "HOME_GERENTE"
...
//        private const val FRAGMENT_HISTORICO = "FRAGMENT_HISTORICO"
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val estabelecimentoCNPJ = AppPreferences.getCNPJLogado(this)
        val edit = userPrefs.edit()
        val statusCaixa = Gson().fromJson(
            userPrefs.getString(
                estabelecimentoCNPJ,
                Gson().toJson(CaixaStatus(status = false, data = LocalDate.now()))
            ),
            CaixaStatus::class.java
        )

        Log.i(logtag, "Caixa fechado: ${statusCaixa.status}")
        if (statusCaixa.data.dayOfMonth == dataHoje.dayOfMonth &&
            ...
            builder.setPositiveButton("SIM"){dialog,_->
                ...
                edit.putString(estabelecimentoCNPJ, Gson().toJson(statusCaixa))
                edit.apply()
                dialog.dismiss()
            }
