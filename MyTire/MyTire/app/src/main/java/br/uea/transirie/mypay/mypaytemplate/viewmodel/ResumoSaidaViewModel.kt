...
//    private val className = "RESUMO_SAIDA_VIEW_MODEL"
    ...
    fun removeServico(servico: Servico) {
        val item = listaItems.find { it.servicoId == servico.id }
        if (item != null) {
            listaItems.remove(item)
            listaServicos.remove(servico)
            atendimento.valorTotal -= servico.preco

            itemAtendimentoRepo.delete(item)
            atendimentoRepo.update(atendimento)
        }
    }

    fun haMaisdeUmEscolhido() = (listaServicos.size > 1)
}