...
class VisualizarColaboradorActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        doAsync {
            ...
            val estab = viewModel.estabelecimentoByCNPJ(usuario!!.estabelecimentoCNPJ)
            uiThread {
    ...
    private fun setAppTopBarEvents(){
        ...
        binding.listaColabTopAppBar.setOnMenuItemClickListener { menuItem ->
            when(menuItem.itemId){
                R.id.editar_dados ->{
                    startActivity(Intent(context, EditarColaboradorActivity::class.java).putExtra(getString(R.string.EXTRA_USUARIO), usuario))
                    true
                }
                else -> false
            }
